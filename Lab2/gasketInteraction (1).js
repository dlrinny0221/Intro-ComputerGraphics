...
*/
	...
	document.getElementById("Randomize").onclick = function () {
		generateRan();
        NumTimesToSubdivide = Math.floor(Math.random() * 12 + 1);
        points = [];
        divideTriangle( vertices[0], vertices[1], vertices[2],
                    NumTimesToSubdivide);
        rgb = 'R';
    ...
    document.getElementById("enter").onclick = function () {
        ...
        if(!isNaN(input)){
        	...
        	else if(input > NumTimesToSubdivide) rgb = 'O';
        	else if (input < NumTimesToSubdivide) rgb = 'Y';
        	refresh();
        }else{
    ...
    canvas.addEventListener("mousemove", function(event){
		...
		var mouseY = (-(event.pageY-159-100-canvas.height/2))/(canvas.height/2);  //159+40=199
...
function refresh(){
    ...
    if(rgb == 'R'){
    	...
    	program = initShaders( gl, "vertex-shader", "fragment-shader-orange" );
    }else if(rgb == 'Y'){
    	program = initShaders( gl, "vertex-shader", "fragment-shader-yellow" );
    }
